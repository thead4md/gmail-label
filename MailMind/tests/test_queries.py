...
def _insert_prediction(
    db: Database,
    gmail_id: str = "test001",
    pipeline_used: str = "rules_only",
    ...
    db.execute_sql(
        """INSERT INTO predictions
           (gmail_id, pipeline_used, primary_label, score, confidence,
            rule_matches, ml_label, ml_confidence, llm_label, llm_confidence, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (gmail_id, pipeline_used, primary_label, score, confidence,
         rule_matches, ml_label, ml_confidence, llm_label, llm_confidence, created_at),
    )
...
def _insert_action(
    ...
    db.execute_sql(
        """INSERT INTO actions (gmail_id, action, status, error, created_at)
           VALUES (?, ?, ?, ?, ?)""",
        (gmail_id, action, status, error, created_at),
    )
...
def _insert_sender_reputation(
    ...
    score: float = 0.5,
    updated_at: int = 1000000,
) -> None:
    db.execute_sql(
        """INSERT INTO sender_reputation (sender, score, updated_at)
           VALUES (?, ?, ?)""",
        (sender, score, updated_at),
    )
...
class TestGetRecentPredictions:
    ...
    def test_single_prediction(self, db: Database) -> None:
        ...
        result = get_recent_predictions(db)
        assert len(result) == 1
        assert result[0]["gmail_id"] == "test001"
        assert result[0]["pipeline_used"] == "rules_only"
    ...
    def test_multiple_predictions_ordered(self, db: Database) -> None:
        ...
        _insert_prediction(db, gmail_id="a", created_at=200)
        _insert_prediction(db, gmail_id="b", created_at=100)
        result = get_recent_predictions(db)
        assert len(result) == 2
        # Most recent first
        assert result[0]["gmail_id"] == "a"
        assert result[1]["gmail_id"] == "b"

    def test_limit(self, db: Database) -> None:
        for i in range(5):
            gid = f"test{i}"
            _insert_email(db, gmail_id=gid)
            _insert_prediction(db, gmail_id=gid, created_at=i)
...
class TestGetRecentActions:
    ...
    def test_limit(self, db: Database) -> None:
        for i in range(5):
            ...
            _insert_action(db, gmail_id=f"test{i}", created_at=i)
